...
├── CLAUDE.md                         # Agent instructions (becomes project-specific after setup)
...
This applies to `lefthook.yml`, `scripts/smoke.sh`, and `scripts/init.sh`. To add support for a new language, add a commented block in each of these three files using the `# [lang]` / `# [/lang]` markers.
...
Features in `progress/` follow a lifecycle: `pending` -> `in_progress` -> `complete` -> merged via PR. The `blocked_by` array expresses dependencies between features. Skills enforce this lifecycle -- `projd-hands-on` won't pick a feature whose blockers aren't complete, and `projd-end` won't mark a feature complete unless smoke tests pass.
...
2. Add a matching block in `scripts/smoke.sh` with `run_check` calls for the same commands.
3. Add a matching block in `scripts/init.sh` with the dependency install command.
...
- Skills call `./scripts/skill-context.sh` for project state. If you need a new kind of context, add a subcommand to that script rather than embedding shell logic in the skill.
...
1. **Smoke test the template itself**: Run `./scripts/validate.sh --strict` from the repo root. This checks that all configuration is valid and smoke tests pass.
...
3. **Test skills**: Create a test project with `setup.sh`, add a feature file to `progress/`, and run through the skill workflow (`/projd-plan`, `/projd-hands-on`, `/projd-end`).
