...
The PreToolUse hook reads `agent.json` on every command. Check that `branch_prefix`, `allow_push`, and `protected_branches` match your intent. You can inspect the hook logic in `.claude/hooks/check-git-policy.sh`. See [hooks.md](hooks.md) for the full architecture.
